          ...
          {profile.uid ? (
            ...
            >
              <img 
                ...
                onError={(e) => {
                  console.error('[BilibiliCard] 头像加载失败');
                  console.error('[BilibiliCard] 原始profile.avatar:', profile.avatar);
                  console.error('[BilibiliCard] 尝试加载的URL:', e.target.src);
                  // 只有在不是默认头像时才替换
            ...
            <img 
              ...
              onLoad={() => {
                console.log('[BilibiliCard] 头像加载成功:', avatarUrl);
              }}
            />
          )}
                  ...
                  <div className="relative w-full aspect-video rounded overflow-hidden mb-1.5 bg-slate-200">
                    {item.thumbnail ? (
                      <img 
                        ...
                        crossOrigin="anonymous"
                        onError={(e) => {
                          console.error('[BilibiliCard] 图片加载失败:', item.thumbnail);
